...
CATEGORY_COLORS = ['#302A7E', '#D0CCE5', '#8884B3'] 
...
def process_data(df, year_range, category_column, line_category_column='None'):
    """Filters and aggregates data for charting, supporting splits for both bars and lines."""
    df = df.copy()
    ...
    # Process Line Data (Split or Aggregate)
...
def generate_chart(final_data, category_column, show_bars, show_line, chart_title, 
    ...
    chart_ax1.set_xticklabels(final_data['time_period'], fontsize=DYNAMIC_FONT_SIZE)
    ...
    if show_line:
        ...
        for idx, l_col in enumerate(line_cols):
            ...
            # Use specific category color if splitting by the same column, else palette
            ...
            if any(actual_mask):
                chart_ax2.plot(x_pos[actual_mask], y_vals[actual_mask], color=l_color, marker='o', 
                               linestyle='-', linewidth=2, markersize=8, label=display_name)
            ...
            if any(is_predicted):
                ...
                chart_ax2.plot(conn_x, conn_y, color=l_color, marker='o', linestyle='--', linewidth=2, markersize=8)
...
st.markdown("""<div style="background: #f5f7fa; padding: 20px; border-radius: 10px; border-left: 5px solid #302A7E; margin: 15px 0;">
    <p style="margin: 0 0 10px 0; font-size: 16px; color: #333;"><strong>Turn any fundraising or grant export into a time series chart – JT</strong></p>
    <a href="https://platform.beauhurst.com/search/advancedsearch/" target="_blank" style="display: inline-block; background: #fff; padding: 10px 16px; border-radius: 6px; border: 1px solid #ddd; color: #302A7E; font-weight: 600; text-decoration: none; font-size: 14px;">🔗 Beauhurst Advanced Search</a>
        ...
        if df_base is not None:
            ...
            if st.checkbox('Enable Stacked Bar', value=False):
                ...
                if category_column != 'None':
                    ...
                    st.session_state['category_colors'] = {c: st.selectbox(f"Color: {c}", list(PREDEFINED_COLORS.values()), index=i%3) for i, c in enumerate(sorted_cats)}
            else:
